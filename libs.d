...
Cell op_sub_int(Cell[] args) {
  static if (debf) {debEnter("[-]");scope (exit) debLeave();}
  return int_cell(-as_int(args[0]));
}
Cell op_sub_float(Cell[] args) {
  static if (debf) {debEnter("[-]");scope (exit) debLeave();}
...
Cell op_listp(Cell[] args) {
  static if (debf) {debEnter("[list?]");scope (exit) debLeave();}
  assert(args.length==1);
  return float_cell(isa(args[0],TList));
}
...
Cell op_symbolp(Cell[] args) {
  ...
  assert(args.length==1);
  return float_cell(isa(args[0],TSymbol));
}
Cell op_nullp(Cell[] args) {
  static if (debf) {debEnter("[null?]");scope (exit) debLeave();}
  assert(args.length==1);
  return float_cell(!(as_list(args[0]).length));
}
...
void add_libs(Env* env) {
  ...
  env_putfun_sigstr(env,"-",fun_cell(&op_sub_int),"(int)","int");
  env_putfun_sigstr(env,"-",fun_cell(&op_sub_float),"(float)","float");
